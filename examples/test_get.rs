...
fn get_request_token(consumer: &Token<'_>) -> Token<'static> {
    let resp = oauth::get::<DefaultRequestBuilder>(api::REQUEST_TOKEN, consumer, None, None).unwrap();
...
fn get_access_token(consumer: &Token<'_>, request: &Token<'_>) -> Token<'static> {
    let resp = oauth::get::<DefaultRequestBuilder>(api::ACCESS_TOKEN, consumer, Some(request), None).unwrap();
...
fn echo(consumer: &Token<'_>, access: &Token<'_>) {
    ...
    let resp = oauth::get::<DefaultRequestBuilder>(api::ECHO, consumer, Some(access), Some(&req_param)).unwrap();
