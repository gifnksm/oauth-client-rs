...
fn get_request_token(consumer: &Token) -> impl Future<Item = Token<'static>, Error = ()> {
    oauth::post(api::REQUEST_TOKEN, consumer, None, None)
        .map_err(|_| ())
        .map(|bytes| {
...
fn get_access_token(
    ...
    oauth::post(api::ACCESS_TOKEN, consumer, Some(request), None)
        .map_err(|_| ())
        .map(|bytes| {
            let resp = String::from_utf8(bytes).unwrap();
...
fn echo(consumer: &Token, access: &Token) -> impl Future<Item = (), Error = ()> {
    ...
    oauth::post(api::ECHO, consumer, Some(access), Some(&req_param))
        .map_err(|_| ())
        .map(move |bytes| {
